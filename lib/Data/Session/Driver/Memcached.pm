...
L<Data::Session::Driver::Memcached> allows L<Data::Session> to manipulate sessions L<Cache::Memcached>.
...
There is no mechanism (apart from memcached's debug code) to get a list of all keys in a cache managed
by memcached, so there is no way to traverse them via this module.
