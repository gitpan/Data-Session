...
L<Data::Session> is typically used by a CGI script to preserve state data between runs of the script.
This gives the end user the illusion that the script never exits.

It can also be used to communicate between 2 scripts, as long as they agree beforehand what session id to use.

See L<Data::Session::CGISession> for an extended discussion of the design changes between L<Data::Session>
and L<CGI::Session>.
...
Beginners understandably confused by the quantity of options should consult the L</Synopsis> for example code.
...
Use C<memcached> for storage. In this case, you must pass an object of type L<Cache::Memcached> to new() as the
...
This says each session is stored in a separate row of a database table using the L<DBD::mysql> database server.
...
This says each session is stored in a separate row of a database table using the L<DBD::ODBC> database connector.
...
This says each session is stored in a separate row of a database table using the L<DBD::Oracle> database server.
...
This says each session is stored in a separate row of a database table using the L<DBD::Pg> database server.
...
Of course, this id must have a true value. L<Data::Session> dies on all values Perl regards as false.
...
This says ids are to be generated by L<Data::UUID>, to generate a 24 (sic) byte long, base-64 encoded, UUID.
...
This name comes from L<Cache::Memcached> even though the external program you run is called memcached.
...
types of storage), and from there as the 3rd parameter to the set() method of L<Cache::Memcached>.
...
See L</Trouble with Exiting> for details.
...
Returns a HTTP header. This means it does I<not> print the header. You have to do that, when appropriate.
...
Simple instructions for installing L<BerkeleyDB> (Oracle and Perl) are in L<Data::Session::Driver::Berkeley>.

Simple instructions for installing L<Cache::Memcached> and memcached are in L<Data::Session::Driver::Memcached>.
...
However, as explained below, there can be problems with your code (i.e. not with L<Data::Session>) such that
this clean-up code is not called, or, if called, it cannot perform as expected.

The general guideline, then, is that you should explicitly call C<flush()> on the session object before your
program exits.
...
The solution is to always create such a object yourself, and to always pass that into L<Data::Session>.
...
In persistent environments such as L<Plack>, FastCGI and mod_perl, your code exits as expected, but the
...
That's because when I create a database (in Postgres) I use "create database d_name owner d_owner encoding 'UTF8';"
and UUID16 simply produces a 16 byte binary value, which is not guaranteed to be or contain a valid UTF8 character.
...
Many thanks are due to all the people who contributed to both L<Apache::Session> and L<CGI::Session>.
